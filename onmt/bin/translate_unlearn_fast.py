...
def train(steps, opt, unlearn=False):
    #load training data in opt as batch?

    loss_func = build_loss_compute(model, tgt_field, opt)
    optimizer = optim.SGD(params, lr=opt.learning_rate)
    for i in range(steps):
        #calculate loss, do step

        src, src_lengths = batch.src
        outputs, attns = model(src, batch.tgt, src_lengths, bptt=False,
        ...
        loss, batch_stats = loss_func(
            ...
            shard_size=1, #should set to just 1?
            unlearn=unlearn)
        if loss is not None:
            #zero grad? #accum gradients?
            optimizer.backward(loss)
            loss.backward()
        else:
            print('None Loss found!')
        optim.step() #?
        continue
